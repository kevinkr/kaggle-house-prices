...
ridge.caret <- train(x=X_train, y=y,
                        method="ridge",
...
print(ridge.caret)
plot(ridge.caret)
mean(ridge.caret$resample$RMSE)

print(varImp(ridge.caret, scale = FALSE))
plot(varImp(ridge.caret, scale = FALSE), main="Variable Importance using Ridge Regression")

resultsTableExport <- cbind(resultsTable,Model="ridge",lowestRmse=mean(ridge.caret$resample$RMSE))
...
write.csv(submission, "Submissions/caret-ridge-v1-1-31-17.csv", row.names = FALSE)
